...
export const WalletProvider: React.FC<WalletProviderProps> = ({ children }) => {
  ...
  const signTransaction = async (transactionXdr: string): Promise<string | null> => {
    ...
    try {
      ...
      console.log('Tentando assinar transação...', {
        address,
      ...
      const { signedTxXdr } = await kit.signTransaction(transactionXdr, {
        address,
        networkPassphrase: Networks.TESTNET // Usar Networks.TESTNET em vez de WalletNetwork.TESTNET
      });
      ...
      let errorMessage = 'Falha ao assinar transação';
      
      if (err.code === -1) {
        errorMessage = 'Erro interno da carteira. Tente reconectar a carteira.';
